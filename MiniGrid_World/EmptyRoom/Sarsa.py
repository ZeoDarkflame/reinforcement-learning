...
try:
    ...
    sar_vals = np.zeros((8,8,4,3))
    print('recreating')


def policy(x,y,dir,epsilon):    # to pick the actions
...
def gen_episode(env,alpha,gamma,epsilon,render = False):    # generates one episode
    ...
    while(not done):
        ...
        sar_vals[x,y,dir,action] += alpha*(rew + gamma*(sar_vals[newx,newy,newdir,policy(newx,newy,newdir,epsilon)]) - sar_vals[x,y,dir,action])
...
# piece of code to run the policy without exploration
'''    
env = gym.make(name)
