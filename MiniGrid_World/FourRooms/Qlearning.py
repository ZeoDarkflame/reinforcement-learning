...
try:
    frq_vals = np.load('frq_vals.npy')
    print('loading')
except:
    frq_vals = np.zeros((11,11,4,3))
    print('recreating')
...
def policy(x,y,dir,epsilon):
    ...
    if randnum < epsilon:
        ...
        return np.argmax(frq_vals[x][y][dir])
...
def gen_episode(env,alpha,gamma,epsilon,render = False):
    ...
    while(not done):
        ...
        frq_vals[x,y,dir,action] += alpha*(rew + gamma*(np.max(frq_vals[newx,newy,newdir])) - frq_vals[x,y,dir,action])
...
if __name__ == "__main__":
    ...
    np.save('frq_vals.npy',frq_vals)

    # saving results
    plt.clf()
    plt.plot(rewards_list)
    plt.show()
    plt.savefig('frq_rewards.png')
    plt.clf()
    plt.plot(steps_list)
    plt.show()
    plt.savefig('frq_steps.png')
    np.save('frq_rewards',np.array(rewards_list))
    np.save('frq_steps',np.array(steps_list))
